...
export default function UsersPage() {
  ...
  const load = async () => {
    ...
    try {
      const [usersResponse, auditsResponse] = await Promise.all([
        getUsers({ limit: 100 }),
        getAuditLogs({ limit: AUDIT_LOG_LIMIT, page: 1 }),
      ]);

      setUsers(usersResponse.data.users || []);
      setAuditLogs(auditsResponse.data.auditLogs || []);
      setAuditPagination(auditsResponse.data.pagination || {
        page: 1,
        limit: AUDIT_LOG_LIMIT,
        total: auditsResponse.data.auditLogs?.length || 0,
        totalPages: 1,
      });
    } catch (error) {
  ...
  const loadAuditPage = async (page) => {
    ...
    try {
      const response = await getAuditLogs({ limit: AUDIT_LOG_LIMIT, page });
      setAuditLogs(response.data.auditLogs || []);
      setAuditPagination(response.data.pagination || {
        page,
        limit: AUDIT_LOG_LIMIT,
        total: response.data.auditLogs?.length || 0,
        totalPages: page,
      });
    } catch (error) {
      toast.error(extractApiError(error, 'Failed to load audit entries.'));
    } finally {
          ...
          <div className="card">
            ...
            <div className="flex items-center justify-between gap-3 mt-5 pt-4 border-t border-gray-100">
              <p className="text-xs text-gray-500">
                Page {auditPagination.page} of {auditPagination.totalPages} / {auditPagination.total} entries
              </p>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => loadAuditPage(auditPagination.page - 1)}
                  disabled={auditLoading || auditPagination.page <= 1}
                  className="inline-flex items-center gap-1 px-3 py-2 text-xs font-semibold text-gray-600 bg-slate-50 hover:bg-slate-100 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <ChevronLeft size={14} />
                  Prev
                </button>
                <button
                  type="button"
                  onClick={() => loadAuditPage(auditPagination.page + 1)}
                  disabled={auditLoading || auditPagination.page >= auditPagination.totalPages}
                  className="inline-flex items-center gap-1 px-3 py-2 text-xs font-semibold text-gray-600 bg-slate-50 hover:bg-slate-100 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Next
